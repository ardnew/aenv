...
// provided options.
func makeConfig(w io.Writer, opts ...pkg.Option[config]) config {
	var c config

	c.mutex = &sync.RWMutex{}

	return pkg.Wrap(pkg.Wrap(c, WithDefaults(w)), opts...)
}

// clone creates a copy of the config with a separate mutex and applies any
// provided options.
func (c config) clone(opts ...pkg.Option[config]) config {
	c.mutex = &sync.RWMutex{}
...
func (c config) handler(opts ...pkg.Option[config]) slog.Handler {
...
func WithDefaults(w io.Writer) pkg.Option[config] {
...
func WithOutput(w io.Writer) pkg.Option[config] {
	return func(c config) config {
		if w == nil {
			w = io.Discard
		}
...
func WithLevel(level Level) pkg.Option[config] {
	return func(c config) config {
		if c.mutex == nil {
			c.mutex = &sync.RWMutex{}
		} else {
...
// for log messages.
func WithFormat(format Format) pkg.Option[config] {
	return func(c config) config {
		if c.mutex == nil {
			c.mutex = &sync.RWMutex{}
		} else {
...
func WithTimeLayout(layout string) pkg.Option[config] {
	return func(c config) config {
...
func WithCaller(enable bool) pkg.Option[config] {
	return func(c config) config {
		if c.mutex == nil {
			c.mutex = &sync.RWMutex{}
		} else {
...
func WithPretty(enable bool) pkg.Option[config] {
	return func(c config) config {
		if c.mutex == nil {
			c.mutex = &sync.RWMutex{}
		} else {
