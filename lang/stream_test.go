...
func TestNewStream_FromReader(t *testing.T) {
	source := `test : { value : 42 }`
	r := strings.NewReader(source)

	ns, err := NewStream(r).GetDefinition("test")
	if err != nil {
		t.Fatalf("GetDefinition failed: %v", err)
	}
...
func TestParser_Caching(t *testing.T) {
	...
	p1 := NewStreamFromString(source)
	p2 := NewStreamFromString(source)
...
// TestReaderNotConsumedUntilAccess verifies that NewStream doesn't
...
func TestReaderNotConsumedUntilAccess(t *testing.T) {
	...
	p := NewStream(trackingReader)

	if readCount > 0 {
		t.Error("NewStream should not consume reader immediately")
	}
